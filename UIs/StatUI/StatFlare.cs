            ...
            if (IsMouseHovering || uiText.IsMouseHovering) {
                ...
                switch (this.stat) {
                    case Players.PlayerStats.Resilience:
                        Main.hoverItemName = $"Resilience\n+{config.ResilienceHealthPerPoint} Life\n+{config.ResilienceDefensePerPoint} Defense";
                        break;
                    case Players.PlayerStats.Quickness:
                        Main.hoverItemName = $"Quickness\n{100*config.QuicknessAccuracyPerPoint}% increased Accuracy\n{100*config.QuicknessEvasionPerPoint}% increased Evasion\n{100*config.QuicknessMovementPerPoint}% increased Movement Speed\n{100*config.QuicknessAttackSpeedPerPoint}% increased Melee Speed";
                        break;
                    case Players.PlayerStats.Potency:
                        Main.hoverItemName = $"Potency\n{100*config.PotencyDamagePerPoint}% increased Damage\n{100*config.PotencyLeechPerPoint}% of Damage Leeched as Life";
                        break;
                }
            }
        }
